...
struct TestEnvironment {
    TestEnvironment(std::vector<T> && keys, uint32_t num_construction_runs = 1, uint32_t num_lookup_runs = 5, uint64_t seed = 0, bool verbose = false)
        : keys(keys)
    ...
    void test(Builder builder) const {
        ...
        std::cerr << " (" << std::round(average_construction_time * 1000.0) / 1000.0 << "s)" << std::endl;
        std::cerr << "Average Space usage: " << std::round(100.0 * space_usage) / 100.0 << " bits/key" << std::endl;
        ...
        for (const T &key : keys) {
            ...
            if (pos >= num_keys) {
                throw std::runtime_error("MPHF contains out of range values");
            }
            sum += pos;
        }
...
template <typename T>
void test_algorithms(const TestEnvironment<T> & testenv, Algorithm const& algorithm) {
    ...
    if (algorithm == PTHash || algorithm == ALL) {
        ...
        testenv.test(typename mphf::PTHashWrapper<pthash::dictionary_dictionary>::Builder(11.0, 0.88));
        testenv.test(typename mphf::PTHashWrapper<pthash::elias_fano>::Builder(6.0, 0.99));
        testenv.test(typename mphf::PTHashWrapper<pthash::dictionary_dictionary>::Builder(7.0, 0.94));
    }
}
...
int main(int argc, char** argv) {
    ...
    parser.add(
        "num_keys", "The number of 64-bit random keys to use for the test. "
        "If it is not provided, then keys are read from the input (one per line).",
        "-n", false);
    parser.add(
        "num_construction_runs", "Number of times to perform the construction. (default: 1)",
        "--num_construction_runs", false);
    parser.add(
        "num_lookup_runs", "Number of times to perform the lookup test. (default: 1)",
        "--num_lookup_runs", false);
    parser.add(
        "verbose", "Verbose output during construction. (default: false)",
        "--verbose", true);
    parser.add(
        "seed", "Seed used for construction. (default: 0)",
        "--seed", false);
    if (!parser.parse()) {
        return 1;
    }
    ...
    uint32_t num_construction_runs = parser.parsed("num_construction_runs") ? parser.get<uint64_t>("num_construction_runs") : 1;
    uint32_t num_lookup_runs = parser.parsed("num_lookup_runs") ? parser.get<uint64_t>("num_lookup_runs") : 1;
    ...
    if (!parser.parsed("num_keys")) {
        ...
        double average_size = std::accumulate(
            keys.begin(),
            keys.end(),
            0.0,
            [](double sum, const std::string& key) { return sum + key.size(); }) / keys.size();
        std::cout << "Read " << keys.size() << " keys, with average length " << std::round(average_size * 100)/100 << std::endl;
        TestEnvironment<std::string> testenv(std::move(keys), num_construction_runs, num_lookup_runs, seed, verbose);
        test_algorithms(testenv, algorithm);
    } else {
        std::cout << "Generating random keys of 64bits" << std::endl;
        if (num_keys == 0) {
        ...
        TestEnvironment<uint64_t> testenv(std::move(keys), num_construction_runs, num_lookup_runs, seed, verbose);
        test_algorithms(testenv, algorithm);
    }
